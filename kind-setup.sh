#!/usr/bin/env bash
# =============================================================================
# kind-setup.sh — One-shot local deployment on Kubernetes (kind)
#
# Only requirement: Docker must be running on your machine.
# kind and kubectl will be installed automatically if missing.
#
# Supports: macOS · Linux · Windows (Git Bash / WSL)
#
# Usage:
#   ./kind-setup.sh          (chmod +x first, or: bash kind-setup.sh)
# =============================================================================
...
install_kind() {
  ...
  install_bin "kind${ext}" "https://kind.sigs.k8s.io/dl/v0.25.0/kind-${OS}-${ARCH}${ext}"
}

install_kubectl() {
  local ext=""; [ "$OS" = "windows" ] && ext=".exe"
  local ver; ver=$(curl -fsSL https://dl.k8s.io/release/stable.txt)
  install_bin "kubectl${ext}" "https://dl.k8s.io/release/${ver}/bin/${OS}/${ARCH}/kubectl${ext}"
}

# ── kubectl wait with retry (handles transient TLS timeouts) ─────────────────
wait_for_pod() {
  ...
  for i in $(seq 1 $attempts); do
    ...
    warn "${name} wait attempt ${i}/${attempts} timed out — retrying..."
  done
  error "${name} failed to become ready after ${attempts} attempts. Check: kubectl describe pod -n ${NAMESPACE} -l app=${label}"
}
...
update_hosts() {
  ...
  if [ "$OS" = "windows" ]; then
    ...
    if grep -qF "ecommerce.local" "$hosts_file" 2>/dev/null; then
      warn "Hosts file already has ecommerce entries — skipping"
    else
      warn "Windows detected — cannot auto-update hosts file."
      warn "Please run the following as Administrator in PowerShell:"
...
# ── 3. kind ───────────────────────────────────────────────────────────────────
step "3/11 · Checking kind"
if command -v kind &>/dev/null; then
  success "kind already installed: $(kind version)"
else
  warn "kind not found — installing automatically..."
  install_kind
fi
...
# ── 5. Kind cluster ───────────────────────────────────────────────────────────
step "5/11 · Setting up kind cluster"
if kind get clusters 2>/dev/null | grep -q "^${CLUSTER_NAME}$"; then
  warn "Cluster '${CLUSTER_NAME}' already exists — skipping"
else
  info "Creating cluster '${CLUSTER_NAME}'..."
  kind create cluster --name "$CLUSTER_NAME" --config kind-config.yaml
  kubectl label node "${CLUSTER_NAME}-control-plane" ingress-ready=true 2>/dev/null || true
...
# ── 7. Pre-load images (best-effort — speeds up pod scheduling) ───────────────
step "7/11 · Pre-loading images into kind node"
info "Pulling and loading infrastructure images (this may take a few minutes)..."
for img in "${INFRA_IMAGES[@]}"; do
  if docker pull "$img" --quiet 2>/dev/null; then
    kind load docker-image "$img" --name "$CLUSTER_NAME" 2>/dev/null \
      && info "Loaded ${img}" \
      || warn "kind load failed for ${img} — will pull at runtime"
  else
    warn "Could not pull ${img} — will pull at runtime"
  fi
done

info "Loading service images (if available locally)..."
for svc in "${SERVICES[@]}" frontend; do
  img="${REGISTRY}/${svc}:latest"
  if docker image inspect "$img" &>/dev/null; then
    kind load docker-image "$img" --name "$CLUSTER_NAME" 2>/dev/null \
      && success "Loaded ${svc}" \
      || warn "kind load failed for ${svc}"
  else
    info "${svc} not found locally — will pull from GHCR at runtime"
  fi
done
...
kubectl create configmap keycloak-realm        -n "$NAMESPACE" --from-file=ecommerce-realm.json=docker/keycloak/import/ecommerce-realm.json              --dry-run=client -o yaml | kubectl apply -f -
kubectl create configmap postgres-init-scripts -n "$NAMESPACE" --from-file=create-all-databases.sql=docker/postgres/init/create-all-databases.sql        --dry-run=client -o yaml | kubectl apply -f -
...
printf "\n  ${YELLOW}Backend services may take an additional 1-2 minutes to fully start.${NC}\n"
